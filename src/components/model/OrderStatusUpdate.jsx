import { updateOrder } from '@/api/AdminOrder';
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50">
            <div className="relative w-11/12 max-w-lg p-6 bg-white shadow-md rounded-md md:w-2/3">
                <div className="flex justify-between items-center mb-4">
                    ...
                    <button
                        type="button"
                        onClick={() => dispatch(setOrderStatus(false))}
                ...
                <div className="flex justify-between py-2">
                    <p className="text-gray-800 font-semibold text-lg"> Total Product</p>
                    <p className={`text-md`} variant="outline">
                        {order?.order_items?.length}
                    </p>
                </div>
                <div className="flex justify-between py-2">
                    <p className="text-gray-800 font-semibold text-lg">Current user Status</p>
                    <p className={` font-medium text-center p-2 rounded-lg h-8 flex items-center justify-center ${order?.status === 'Pending' ? 'bg-green-50 text-black-700' : order?.status === 'Rejected' ? 'bg-red-200 text-red-700' : 'bg-green-200 text-green-700'}`}>
